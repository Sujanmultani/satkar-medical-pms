    ...
    const batchesProcessed = batches.map((b) => {
      ...
      const batchReturns = returns.filter(
        (r) =>
          ...
          (r.batchId && r.batchId.batchNo && b.batchNo && r.batchId.batchNo.trim().toLowerCase() === b.batchNo.trim().toLowerCase())
      );
      const returnedQty = batchReturns.reduce((sum, r) => sum + (r.quantity || r.qty || 0), 0);
